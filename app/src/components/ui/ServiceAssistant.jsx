...
const ServiceAssistant = ({ maintenanceData, onViewDetail }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end space-y-4">
      {/* EXPANDED WINDOW */}
      <div
        className={`origin-bottom-right transform overflow-hidden rounded-4xl border border-line bg-card shadow-soft-lg transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-hair p-4">
          ...
          <button onClick={() => setIsOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-xl bg-base text-ink2 shadow-soft-in-sm transition active:scale-95">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="max-h-[60vh] space-y-4 overflow-y-auto p-4">
          <div className="flex gap-3">
            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-base text-ink2 shadow-soft-in-sm">
              <Wrench className="h-4 w-4" />
            </div>
            <div className="rounded-2xl rounded-tl-none bg-base p-3 text-sm text-ink2 shadow-soft-in-sm">
              <p>Halo boss! 👋 Jangan lupa ya, motor kesayangan lu ada jadwal bentar lagi.</p>
            </div>
          </div>

          <div className="ml-11 rounded-2xl border border-line bg-card p-4 shadow-soft-sm">
